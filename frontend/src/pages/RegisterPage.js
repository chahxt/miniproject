...
//       navigate('/login'); // You can change this to redirect to login or any other page
//     } catch (error) {
//       // Handle errors if the registration fails (e.g., user already exists, etc.)
...
function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // After successful registration, navigate to the login page
      navigate('/app/login'); // <--- Yahi correct path hai!
    } catch (error) {
      // Handle errors if the registration fails (e.g., user already exists, etc.)
      alert('Registration failed: ' + (error.response?.data?.error || 'Unknown error'));
    }
